...
from create import create_envs, create_threads, ActGroupPool
from eval import evaluate
import common_utils
import rela
import r2d2 as r2d2_pretrained
...
def parse_args():
    ...
    parser.add_argument("--num_update_between_reflection", type=int, default=5000)
...
if __name__ == "__main__":
    ...
    agent.belief_module.load_state_dict(torch.load("model$_multi6op_odds_bothplayers.pth"))
    ...
    for j in range(len(train_pool)):
         partner_agent = r2d2_pretrained.R2D2Agent(
            (args.method == "vdn"),
            args.multi_step,
            args.gamma,
            args.eta,
    ...
    test_agent = r2d2_pretrained.R2D2Agent(
       (args.method == "vdn"),
       args.multi_step,
       args.gamma,
       args.eta,
    ...
    eval_runners1 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[0],"cuda:0",  80, ["act"])
    ]
    eval_runners2 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[1],"cuda:0",  80, ["act"])
    ]
    eval_runners3 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[2],"cuda:0",  80, ["act"])
    ]
    eval_runners4 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[3],"cuda:0",  80, ["act"])
    ]
    eval_runners5 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[4],"cuda:0",  80, ["act"])
    ]
    eval_runners6 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(partner_agents[5],"cuda:0",  80, ["act"])
    ]
    eval_runners7 = [
        rela.BatchRunner(eval_agent, "cuda:0", 80, ["act"]),
        rela.BatchRunner(test_agent,"cuda:0",  80, ["act"])
    ]
...
#                    context.resume()
            ...
            # loss, priority, belief_losses = agent.loss(batch, args.pred_weight, stat)
